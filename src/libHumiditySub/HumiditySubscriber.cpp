...
void HumiditySubscriber::SubListener::on_data_available(DataReader* reader)
{
    ...
    if (reader->take_next_sample(&humidity_, &info) == ReturnCode_t::RETCODE_OK)
    {
        if (info.instance_state == ALIVE_INSTANCE_STATE)
        {
            ...
            //APP_LOG("humidity message received, time captured (t1)=%u, time sent msg out (t2)=%u, time received msg (t3)=%u", humidity_.t1(), humidity_.t2(), APP_TIME_CURRENT_MS);

            file_ << humidity_.index() << "," << APP_TIME_CURRENT_MS - humidity_.publisher_send_time() << std::endl;
        }
    }
}
