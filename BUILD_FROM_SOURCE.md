...
1. set up dependencies
	...
	git clone --jobs $(nproc --all) --depth=1 --branch 1.14.0 https://ceres-solver.googlesource.com/ceres-solver
	cd ceres-solver
	mkdir build
	cd build
	cmake ..
	...
	mkdir opencv
	cd opencv
	git clone --jobs $(nproc --all) --depth=1 --single-branch --branch 4.1.1 --recursive https://github.com/opencv/opencv
	...
	*Note*: This is specifically suppose to be for opencv 4.1.1.
	...
	4. Compile the fastDDS library
		...
		3. Install it
		```bash
		sudo ./install.sh --build-cores $(nproc --all)
		```

		So the following should be the process of installing fast DDS
		```bash
		git clone --jobs $(nproc --all) --depth=1 --single-branch --branch=2.12.1 --recursive https://github.com/eProsima/Fast-DDS fastDDS
		sudo ./install.sh
		# Alternative
		# mkdir fastDDS
		# cd fastDDS
